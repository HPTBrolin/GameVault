# Executar a partir de C:\Proj\GameVault\web
#   pwsh -File .\fix-gamevault-router-ui.ps1
...
# 1) main.tsx – um único RouterProvider
$main = @'
...
// Páginas (lazy opcional; aqui simples para estabilidade)
...
# 2) App.tsx – layout + <Outlet />, NENHUM <Router> aqui
$app = @'
...
export default function App(){
  ...
  return (
    <div className="app-shell" style={{background:"#0f1720", minHeight:"100vh", color:"#E6EDF3"}}>
...
# 3) http.ts – compatível com apiGet/apiPost e get/post
$http = @'
...
export function put<T = any>(url: string, data?: any, config: any = {}) {
...
# 5) Library.tsx – usa listGamesPaged e mantém sort/order na URL
$lib = @'
import React from "react";
  ...
  return (
    <div>
      <div style={{display:"flex", gap:8, alignItems:"center", marginBottom:12}}>
        ...
        <button onClick={()=>alert("Sidebar de filtros por implementar aqui (UI apenas).")}>Filtros</button>
...
# 6) Add.tsx (placeholder funcional)
$add = @'
import React from "react";
...
export default function Settings(){
  ...
  return (
    <div>
      ...
      {tab==="general" && <div>Configurações da aplicação (logs, segurança, URL base).</div>}
...
# 8) GameDetail.tsx – detalhe básico
$detail = @'
import React from "react";
...
Write-Host "`nAll done. Reinicia o dev server (se estiver a correr)."
'@

Write-Host "`n"
Write-Host "⚡ Agora corre:"
Write-Host "   pwsh -File .\fix-gamevault-router-ui.ps1"
Write-Host "e depois reinicia o 'npm run dev'." -ForegroundColor Green
